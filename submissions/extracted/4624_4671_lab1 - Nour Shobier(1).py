...
class TftpProcessor(object):
    ...
    def upload_file(self, file_name):
        ...
        # Opcode(2b)(int) | Filename(string) | 0(1b)(int) | Mode(string) | 0(1b)(int)
...
def do_socket_logic(udp_socket, operation: str, server_address, file_name: str):
    ...
    if operation == "push":
        request_packet = tftp_processor.upload_file(file_name)
    else:
        request_packet = tftp_processor.request_file(file_name)

    udp_socket.sendto(request_packet, server_address)
    packet, address = udp_socket.recvfrom(4096)
    tftp_processor.process_udp_packet(packet, address)

    try:
        while tftp_processor.has_pending_packets_to_be_sent():
            data_packet = tftp_processor.get_next_output_packet()

            udp_socket.sendto(data_packet, address)
            packet, address = udp_socket.recvfrom(4096)
